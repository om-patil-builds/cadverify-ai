...
const ComparePage = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setMessage(`Upload complete: ${response.stored_filenames?.join(', ')}`);
      setPdfFile(null);
      setDxfFile(null);
      setUploadProgress(0);
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-8 shadow-lg shadow-slate-950/30">
        ...
        <p className="mt-3 max-w-2xl text-slate-400">
          Upload a scanned PDF drawing and its DXF counterpart to prepare them for future comparison workflows.
        </p>
      </div>
      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-8 shadow-lg shadow-slate-950/30">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <label className="rounded-2xl border border-slate-800 bg-slate-950/60 p-5">
            ...
            <input type="file" accept=".pdf" className="mt-4 block w-full text-sm text-slate-400" onChange={(event) => handleFileSelection(event, 'pdf')} />
          ...
          <label className="rounded-2xl border border-slate-800 bg-slate-950/60 p-5">
            ...
            <input type="file" accept=".dxf" className="mt-4 block w-full text-sm text-slate-400" onChange={(event) => handleFileSelection(event, 'dxf')} />
        ...
        <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="text-sm text-slate-400">
            {isUploading ? `Progress: ${uploadProgress}%` : 'Upload button is enabled once both files are selected.'}
          </div>
        </div>
